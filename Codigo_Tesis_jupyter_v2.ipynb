# -*- coding: utf-8 -*-
"""
Created on Tue Jan  7 22:41:58 2025
...
def obtener_opciones(portfolio, start_date, end_date):
    ...
    El DataFrame contiene las siguientes columnas:
        ...
        - 'currency' : Indica la moneda en la que se emite la opción
    ...
    var_final=["contractSymbol", "lastTradeDate",	"strike",	"lastPrice", "impliedVolatility",	"optionType",	"symbol",	"shortDate", "inTheMoney",	"currency"]
    return final_df[var_final]
...
def longstaff_schwartz(paths, strike, r, option_type, symbol, graphics=False):
    """
    ...
    symbol: Nombre de la acción

    graphics: Valor tipo booleano (True o False) por defecto es False
    ...
    if graphics:
        ...
        for i in range(paths.shape[0]):  # Mostrar las trayectorias
            ax.plot(t, paths[i, :], label=f'Trayectoria {i+1}')
        
        ax.set_title(f"Trayectorias de la Acción {symbol} a lo largo del tiempo")
        ax.set_xlabel("Tiempo en t")
        ax.set_ylabel(f"Precio de la acción {symbol}")
        ax.grid(True)
        plt.show()
    
        # Crear gráfico de cajas con Seaborn
        sns.set(style="whitegrid")  # Estilo con fondo blanco y rejilla
        fig, ax = plt.subplots(figsize=(8, 6))
        sns.boxplot(data=final_cfs['present_value'], ax=ax)
        ax.set_title(f"Gráfico de Cajas del Precio de la Opción {option_type.capitalize()}")
        ax.set_xlabel(f"Distribución de los precios de la Opción {option_type.capitalize()}")
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(exercise_times_normalized, exercise_prices, "rx", label="Ejercicio")
        ax.plot(continuation_times_normalized, continuation_prices, ".", color="grey", label="Continuación")
        ax.legend()
        ax.set_xlabel("Tiempo t")
        ax.set_ylabel(f"Precio de la acción {symbol}")
        ax.set_title(f"Ejercicio vs Continuación - Opción {option_type.capitalize()}")
        ax.grid(True)
        plt.show()
        ...
        # Etiquetas y leyenda
        plt.xlabel("Tiempo t")
        plt.ylabel(f"Precio de la acción {symbol}")
        plt.title(f"Trayectorias con primer ejercicio de la Opción {option_type.capitalize()}")
...
def price_value_op( symbol, optionType, strike, impliedVolatility, lastPrice, T, dt, num_paths, rateRiskFree,graphics):
    ...
    option_price, lower_interval, upper_interval, option_valuation, vector_valuation = longstaff_schwartz(
        paths=paths, strike=strike, r=rateRiskFree, option_type=optionType, symbol=symbol, graphics=graphics
    )
...
# Filtrar las filas donde 'symbol' está en el vector portfolio
filtered_df = df[df['symbol'].isin(portfolio)]
# Calcular la diferencia entre 'optionPrice' y 'monte_carlo_price'
filtered_df['price_diff'] = filtered_df['optionPrice'] - filtered_df['monte_carlo_price']

# Crear un diccionario para almacenar los contractSymbols correspondientes al máximo y mínimo
contract_symbols = {}

# Iterar sobre cada símbolo en el portfolio
for symbol in portfolio:
    # Filtrar el DataFrame para cada símbolo
    symbol_df = filtered_df[filtered_df['symbol'] == symbol]
    
    # Obtener el máximo y mínimo de la diferencia para ese símbolo
    max_diff = symbol_df['price_diff'].max()
    min_diff = symbol_df['price_diff'].min()
    
    # Filtrar las filas donde la diferencia es igual al máximo
    max_row = symbol_df[symbol_df['price_diff'] == max_diff]
    min_row = symbol_df[symbol_df['price_diff'] == min_diff]
    
    # Almacenar los contractSymbols en el diccionario
    contract_symbols[symbol] = {
        'max_contractSymbol': max_row['contractSymbol'].values.tolist(),
        'min_contractSymbol': min_row['contractSymbol'].values.tolist()
    }

# Crear una lista vacía para almacenar los contractSymbols de máximo y mínimo
max_contractSymbols = []
min_contractSymbols = []

# Extraer los contractSymbols de máximo y mínimo de cada símbolo
for symbol, contracts in contract_symbols.items():
    max_contractSymbols.extend(contracts['max_contractSymbol'])
    min_contractSymbols.extend(contracts['min_contractSymbol'])

# Filtrar el DataFrame original para obtener las filas correspondientes
final_df = df[df['contractSymbol'].isin(max_contractSymbols + min_contractSymbols)]
